...
async function proxyAuditRequest(request: NextRequest, path: string[], method: string) {
  const accessToken = await getOptionalAccessToken();
  const targetUrl = new URL(`${apiBaseUrl}/api/audit/${path.join('/')}`);
...
export async function GET(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  return proxyAuditRequest(request, path, 'GET');
}
