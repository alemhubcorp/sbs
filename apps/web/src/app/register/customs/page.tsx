...
export default async function CustomsRegisterPage({ searchParams }: { searchParams?: SearchParams }) {
  const resolved = (await searchParams) ?? undefined;
  const returnTo = readReturnTo(resolved);
  redirect(`/signin?mode=register&role=customs&returnTo=${encodeURIComponent(returnTo)}`);
}
